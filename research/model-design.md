...
including every winning slot and copied value. Its 32-by-16 interface exposes
a direct edit to one value byte in context RAM, confirms the record before and
after, and labels the model weights locked. Asking the same query before and
after the edit changes the copied value without changing the matching weights;
an explicitly paced score replay is optional. Real-ROM XRoar reaches the
keyboard loop; physical timing and keyboard behaviour remain unmeasured.
