...
def main() -> None:
    ...
    lines += [
        ...
        "        sta     firstanswer",
        "        lda     #$02",
        f"        sta     ${address['attention_ui_edit_old_value']:04x}",
        f"        jsr     ${address['attention_ui_draw_edit']:04x}",
        f"        lda     ${SCREEN:04x}",
        "        sta     edittitle",
        "        lda     #$06",
        f"        jsr     ${address['attention_ui_apply_edit']:04x}",
        f"        jsr     ${address['attention_ui_draw_edit_done']:04x}",
        f"        lda     ${SCREEN:04x}",
        "        sta     changedtitle",
        f"        lda     ${address['attention_query']:04x}",
        "        sta     keptquery",
        f"        lda     ${address['attention_ui_selected_slot']:04x}",
        "        sta     keptslot",
        f"        lda     ${address['attention_memory_values'] + 1:04x}",
        "        sta     editedbinding",
        f"        lda     ${address['attention_query_weights']:04x}",
        "        sta     queryfirst",
        f"        lda     ${address['attention_query_weights'] + 79:04x}",
        "        sta     querylast",
        f"        lda     ${address['attention_key_weights']:04x}",
        "        sta     keyfirst",
        f"        lda     ${address['attention_key_weights'] + 79:04x}",
        "        sta     keylast",
