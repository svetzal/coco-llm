...
def pack_signed_nibbles(values: bytes) -> bytes:
    signed = np.frombuffer(values, dtype=np.int8)
    if np.any(signed < -8) or np.any(signed > 7):
        raise ValueError("packed model value does not fit signed four-bit")
    packed = bytearray()
    for offset in range(0, len(signed), 2):
        high = int(signed[offset]) & 0x0F
        low = int(signed[offset + 1]) & 0x0F if offset + 1 < len(signed) else 0
...
def main() -> None:
    ...
    image = fixed.model_bytes()
    packed = pack_signed_nibbles(image)
    ...
    for weights, bias in zip(
        ...
        minimum_score = int(bias) * 4
        maximum_score = int(bias) * 4
    ...
    (output / "weights-packed.bin").write_bytes(packed)
    ...
    assembly = [
        ...
        f"EXP7_PACKED_BYTES      equ     {len(packed)}",
    ...
    packed_model = [
        "; Generated by tools/export_exp_007.py. Do not edit.",
        "exp7_packed_model",
        *assembly_bytes(packed),
        "exp7_packed_model_end",
        "",
    ]
    (output / "packed_model.inc").write_text("\n".join(packed_model), encoding="ascii")
    ...
    manifest = {
        "experiment": 7,
        "format": "signed Q2.2 nibbles expanded to signed bytes",
        "layout": offsets,
    ...
    print(
        f"wrote {len(image)} expanded bytes and {len(packed)} packed bytes to {output}"
    )
