...
Here is the second table after the 20 epochs, drawn as 29 points:

![Twenty-eight points inside a wireframe box drawn on the CoCo's green screen, titled 28 TOKENS AS 28 POINTS. Each point has a dashed drop line to the box's floor so its depth reads, and is labelled with its token. The six makers, ACORN, APPLE, ATARI, COMMODORE, SINCLAIR and TANDY, are larger amber points and sit in one region of the box; the model names are small black points bunched on the left, and COLOR, BBC, ZX and MODEL sit high up by themselves. A panel lists straight-line distances: APPLE to ATARI 0.12, COMMODORE to ATARI 0.44, COMMODORE to 64 1.13, COMMODORE to TANDY 1.16. A note says END is left out because its point sits far above the box. A caption reads: nothing in the numbers says what a maker is; the makers are near each other because the same kind of token follows them.](images/twenty-nine-points.png)
