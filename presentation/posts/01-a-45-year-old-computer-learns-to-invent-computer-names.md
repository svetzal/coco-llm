...
![A diagram titled Two words at a time. Across the top, the name COMMODORE AMIGA as five boxes in a row: END, END, COMMODORE, AMIGA, END. Below it the same row three times, labelled example 1, 2 and 3. In each, a pale amber outline marks a window two boxes wide, and the box just to its right is filled solid amber as the token that came next: the window over END END points at COMMODORE, the window over END COMMODORE points at AMIGA, and the window over COMMODORE AMIGA points at END. Boxes past the window are greyed out. A legend below reads: the context window, two tokens wide; the token that came next. A last line says everything before the window is gone.](images/two-words-at-a-time.png)
